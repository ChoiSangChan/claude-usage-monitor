...
MENUBAR_SRC="$INSTALL_DIR/menubar/claude-usage-monitor.5m.py"

if [ -f "$MENUBAR_SRC" ]; then
    if [ -d "$XBAR_PLUGIN_DIR" ]; then
        cp "$MENUBAR_SRC" "$XBAR_PLUGIN_DIR/"
        chmod +x "$XBAR_PLUGIN_DIR/claude-usage-monitor.5m.py"
...
echo "  xbar 플러그인: $XBAR_PLUGIN_DIR/claude-usage-monitor.5m.py"
echo ""
